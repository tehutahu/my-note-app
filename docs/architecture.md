...
### IndexedDB schema2（2026-09-08）

端末内DBのversionを2とし、pagesに非unique索引byNotebook（keyPath notebookId）を追加する。ノートを開く時はこの索引でページを取得し、notebook.pageIdsの順序を復元する。専用バックアップ形式のschemaVersionは1を維持し、DBのversionとは区別する。移行はversionchange transaction内だけで実行する。blockedで画面へ失敗を通知したopen要求は、後から旧接続が閉じても移行をabortして、再起動時の新しい要求で改めて行う。

### 診断計測（2026-09-09）

「動作の計測」から開始/停止し、本文を含まないJSONを保存する。項目ごとの最新10,000件の生値と、開始以来の総件数・総失敗数を保持する。成功サンプルの中央値/p95と失敗数を分ける。再開始時に前回をリセットし、開始前から進行中の処理は混ぜない。OSの温度/省電力は自動推測せずユーザーが選択する。

保存開始/commitは筆記pointerup handler冒頭（その他の編集はsession.edit開始）から測る。保存queueは最初の未保存編集時刻を保持し、失敗/再試行で待ち時間を隠さない。入力は実行handler開始からCanvas処理終了までであり物理ペンの総遅延ではない。PDF表示は現在ページのCanvas反映までを測り、次ページに置き換えられた古い描画は成功/失敗に数えない。

確定Canvasはページ・背景・カメラ・解像度が同じで要素列の末尾だけが増えたとき、追加要素だけ描画する。削除/Undo/ズーム/ページ移動等は全再描画する。下書きは別Canvasのまま。保存用snapshotはeditで深いコピーを1回行い、その後のsaveは独立したnotebookメタデータと変更しないpagesを利用する。
